...
test('malformed rows fail closed',()=>{const r=summarize({generation:[null,'secret',{state:{leak:true},jd_source:'bad value'}],render_jobs:[null,{state:'pending'}],material_versions:[null,{state:'claimed',lease_expires_at:'not-a-date'}]});assert.equal(r.counts.generation,1);assert.equal(r.render.queue_depth,1);assert.equal(r.stale_lease.materials,0);assert.deepEqual(r.jd_source,{unknown:1});});
