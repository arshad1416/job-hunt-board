...
export const REQUIRED_ENV = ['TURSO_URL', 'TURSO_TOKEN', 'R2_ENDPOINT', 'R2_ACCESS_KEY_ID', 'R2_SECRET_ACCESS_KEY', 'R2_BUCKET'];
// Pages-side configuration the Pi render worker never reads; reported for
// operators but not gating, so a render host without the LLM/dashboard
// secrets still passes.
export const PAGES_ENV = ['NINEROUTER_API_KEY', 'DASHBOARD_AUTH_TOKEN', 'JOB_MATERIALS_BUCKET'];
...
export function runPreflight(env = process.env, commandExists = (command) => { try { execFileSync('which', [command], { stdio: 'ignore' }); return true; } catch { return false; } }, chromium = chromiumPath) {
  ...
  const pages = Object.fromEntries(PAGES_ENV.map((name) => [name, Boolean(env[name]?.trim())]));
  ...
  return { ok: Object.values(checks).every(Boolean) && Object.values(commands).every(Boolean), checks, pages, commands };
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = runPreflight();
  if (process.argv.includes('--json')) console.log(JSON.stringify(result));
  else { for (const [name, present] of Object.entries(result.checks)) console.log(name + ': ' + (present ? 'configured' : 'missing/invalid')); for (const [name, present] of Object.entries(result.pages)) console.log(name + ' (pages-only): ' + (present ? 'configured' : 'missing')); for (const [name, present] of Object.entries(result.commands)) console.log(name + ': ' + (present ? 'available' : 'missing')); }
