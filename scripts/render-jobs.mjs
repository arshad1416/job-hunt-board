...
export function parseWorkerArgs(argv=[]){const out={dryRun:true,limit:10};for(let i=0;i<argv.length;i++){if(argv[i]==='--dry-run')out.dryRun=true;else if(argv[i]==='--no-dry-run'||argv[i]==='--execute')out.dryRun=false;else if(argv[i]==='--limit'){const n=Number(argv[++i]);if(!Number.isInteger(n)||n<1||n>50)throw new Error('invalid --limit');out.limit=n}else throw new Error('unknown argument '+argv[i])}return out}
if(import.meta.url===`file://${process.argv[1]}`){try{const args=parseWorkerArgs(process.argv.slice(2)),env=process.env;if(!env.TURSO_URL||!env.TURSO_TOKEN)throw new Error('TURSO_URL and TURSO_TOKEN are required');if(!args.dryRun){for(const k of ['R2_ENDPOINT','R2_ACCESS_KEY_ID','R2_SECRET_ACCESS_KEY','R2_BUCKET'])if(!env[k])throw new Error('missing '+k);if(!chromiumPath())throw new Error('chromium unavailable')}const options={env,args};if(!args.dryRun){options.bucket=createR2S3(env);options.renderer=createLocalPdfRenderer(env)}runWorker(options).then(x=>{console.log(JSON.stringify({dryRun:x.dryRun,count:x.count??x.claimed?.length??0,results:x.results?.map(r=>({id:r.id,state:r.state}))??[]}));if(x.ok===false)process.exitCode=1}).catch(e=>{console.error(e.message);process.exitCode=1})}catch(e){console.error(e.message);process.exitCode=1}}