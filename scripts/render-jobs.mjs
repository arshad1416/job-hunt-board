...
if(import.meta.url===`file://${process.argv[1]}`){const args=parseWorkerArgs(process.argv.slice(2));if(!process.env.TURSO_URL||!process.env.TURSO_TOKEN)throw new Error('TURSO_URL and TURSO_TOKEN are required');if(!args.dryRun)throw new Error('production processing requires an injected bucket and renderer');console.log(JSON.stringify(await pollRenderJobs({env:process.env,...args})));}
