...
export async function processRenderJob(row, { env, dryRun = false, execute = tursoExecute, query = tursoQuery, bucket, renderer } = {}) {
  ...
  try {
    ...
    const uploaded = [];
    try {
      for (const [pdf, name] of staged) { await leaseLive(); uploaded.push(prefix + '/' + name); await bucket.put(prefix + '/' + name, pdf.body, { onlyIfNew: true });
        await leaseLive(); }
    } catch (error) {
