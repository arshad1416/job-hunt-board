...
export async function processRenderJob(row, { env, dryRun = false, execute = tursoExecute, query = tursoQuery, bucket, renderer } = {}) {
  ...
  try {
    ...
    if (error.message === 'lease_stale') return { id, state: 'stale' };
    // Transport-level failures (R2/Turso network errors, SigV4 faults) are
    // infrastructure, not document defects: surface them so the worker exit
    // code distinguishes "job is bad" from "the pipeline is down".
    if (!JOB_FAILURE_CODES.has(error.message)) return { id, state: 'infra_error', error: String(error.message).slice(0, 120) };
    return terminal(false, error.message);
  }
}
// Errors that mean this job's inputs or gates are defective (retrying other
// jobs is fine). Everything else is treated as infrastructure failure.
export const JOB_FAILURE_CODES = new Set(['source_prefix_missing', 'source_prefix_invalid', 'source_missing', 'manifest_invalid', 'source_tampered', 'pdf_gates_failed', 'pdf_render_failed', 'renderer_failed', 'renderer_input_incomplete', 'chromium_unavailable', 'pdf_exists']);
