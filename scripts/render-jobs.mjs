...
export function createLocalPdfRenderer(env={}){const templates=loadTemplates();return async(markdown,type,details={})=>{if(!['name','email','phone','location'].every(k=>details[k]))return{ok:false,error:'renderer_input_incomplete'};const browser=env.CHROMIUM_BIN||chromiumPath();if(!browser)return{ok:false,error:'chromium_unavailable'};const data={...details,resume:{...details,markdown:type==='resume'?markdown:''},coverLetter:{...details,markdown:type==='cover_letter'?markdown:''}};const rendered=renderMaterials(data,templates);const pdf=renderPdf(type==='resume'?rendered.resume:rendered.cover,{type,browser});if(!pdf.ok)return{ok:false,error:'pdf_render_failed'};return{ok:true,body:new Uint8Array(fs.readFileSync(pdf.pdf)),pages:pdf.pages,gates:pdf.gates};};}
