...
export async function processRenderJob(row, { env, dryRun = false, execute = tursoExecute, query = tursoQuery, bucket, renderer } = {}) {
  ...
  const validPrefix = parts.length === 5 && parts[0] === 'materials' && parts[1] === String(jobId) && parts[2] === 'versions' && parts[3] === String(version).toLowerCase() && parts[4] === token && /^\d+$/.test(parts[1]) && /^[a-f0-9]{64}$/.test(parts[3]);
  const leaseLive = async () => {
    if (bucket?.checkLease) { if (!(await bucket.checkLease(row))) throw new Error('lease_stale'); return true; }
