...
if(import.meta.url===`file://${process.argv[1]}`){const args=parseWorkerArgs(process.argv.slice(2));if(!process.env.TURSO_URL||!process.env.TURSO_TOKEN)throw new Error('TURSO_URL and TURSO_TOKEN are required');console.log(JSON.stringify(await pollRenderJobs({env:process.env,...args})));}
