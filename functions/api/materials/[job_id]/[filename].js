...
export async function onRequestGet(context) {
  ...
  if (version) { const material = await getMaterialVersion(env, jobId, version); if (!material || material.state !== 'succeeded' || !material.source_exists || !material.hard_gates_pass || material.artifact_prefix !== `materials/${jobId}/versions/${version.toLowerCase()}/attempt-${String(material.artifact_prefix || '').split('/attempt-')[1]}` || !/^materials\/\d+\/versions\/[a-f0-9]{64}\/attempt-[A-Za-z0-9_-]+$/.test(material.artifact_prefix || '')) return new Response(JSON.stringify({ error: 'Unverified material version is unavailable' }), { status: 404, headers: { 'Content-Type': 'application/json' } }); artifactPrefix = material.artifact_prefix; }
