...
export async function createProfileManifest(profile, {sourceType='text', objectHashes={}}={}){const text=validateProfile(typeof profile==='string'?profile:JSON.stringify(profile));const revision=await profileRevision(text);return{schema:PROFILE_SCHEMA,revision,content_sha256:revision,bytes:new TextEncoder().encode(text).length,source_type:sourceType,object_hashes:objectHashes}}
