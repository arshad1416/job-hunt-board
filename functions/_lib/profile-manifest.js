...
export async function writeIfNew(bucket,key,body,{confirm=false,onlyIfNew=true}={}){if(!validatePrivateKey(key))throw new Error('private key is not allowed');if(!bucket?.head||!bucket?.put)throw new Error('private bucket is required');const revision=await sha256Hex(body);if(!confirm)return{status:'dry-run',key,revision};if(onlyIfNew&&await bucket.head(key))return{status:'unchanged',key,revision};await bucket.put(key,body,{onlyIfNew});return{status:'written',key,revision}}
