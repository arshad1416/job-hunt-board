...
export function validateProfileManifest(m){return Boolean(m&&m.schema===PROFILE_SCHEMA&&/^[a-f0-9]{64}$/.test(m.revision||'')&&m.revision===m.content_sha256&&Number.isInteger(m.bytes)&&m.bytes<=MAX_INPUT_BYTES&&(!m.object_hashes||Object.values(m.object_hashes).every(x=>/^[a-f0-9]{64}$/.test(x))))}
